...
def _build_user_message(plate_dict: dict) -> str:
    ...
    msg_parts.append("")
    msg_parts.append("**⚠️ 强制要求——验盘前先做流年全扫描**：在给出任何预测之前，必须先列出当前大运窗口内每一年的流年信号表（年份/干支/与四柱关系/信号等级），标注S/A/B/C/D/E等级。特别标注：禄神到位年、印星齐透年、大运交接年、天克地冲年——这几类即使信号等级非S/A也必须进候选。全表输出后，从中选信号最强的3个作为验盘预测。禁止不列表直接给预测——这是最常见的漏信号原因。")
    msg_parts.append("")
