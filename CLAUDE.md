...
## Agent 优化策略（2026-06-16 盲测驱动，2026-06-24 迭代）
...
- 错误模式：13条（盲测驱动 + 2026-06-24 迭代）
- 流年信号优先级表：S/A/B/C/D/E 6级
- 叙事等级制度：1-5级
- 禁用戏剧化词：7个
- 验盘策略：极端度分流 + 时间线验证
- **均衡命局自适应降级（2026-06-24 新增）**：验盘条数 = min(3, 可用信号数)，流年干支-西历对照表注入 user message，Agent 从表引用禁止心算，信号不足时诚实告知
- **大运状态标记规则（2026-06-24 新增）**：对照表中每步大运标注 🔵当前/✅已走完/⬜未开始，Agent 选年优先选已走完的大运
- **Gate Check + CoVe 强化（2026-06-24）**：主 Gate Check 第 7 项触发均衡命局专用检查，CoVe 模板新增第 4 项"硬凑检查"
- **置信度标签颜色修正（2026-06-24）**：🔴高=S/A | 🟡中=B/C | 🔵低=D/E/大运交接，模式 4.1 按实际信号等级动态匹配

### 对照表注入管道（2026-06-24 新增）

`analysis_service.py` 新增两个函数：
- `_evaluate_liunian_signal(liunian_ganzhi, ri_ganzhi, yue_zhi, nian_zhi, dayun, year)` → `(level, desc)` — 六级信号判定
- `_build_year_lookup_table(plate_dict, current_year)` → markdown 字符串 — 逐年全量对照表（出生年→当前年）
