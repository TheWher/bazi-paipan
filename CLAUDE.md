...
| 紫微 | `.claude/agents/ziwei-master.md` | ~525 | 10KB | 三合中州派·9步推理链·四层四化权重·8格局核验·风险过滤 |
...
### 紫微斗数 Agent 定义统计（2026-07-17：v6）

- 路径：`.claude/agents/ziwei-master.md`
- 大小：525 行 / 10KB
- 推理链：9 步强制推理（观命宫→查身宫→定格→扫四化→查煞忌→联宫位→判大限→看流年→综合）
- 核心方法论：
  - **亮度体系**：庙旺得利平不陷 7 级，含 iztro 5 级映射
  - **四层四化权重**：生年(★★★★★)/大限(★★★★)/本宫自化(★★★)/流年(★★)，15 条互涉规则
  - **身宫独立框架**：4 维分析 + 4 段年龄权重表 + 命身同宫处理
  - **对宫冲照**：6 组对宫分析维度 + 7 级冲照力度
  - **格局系统**：8 核心格局(必要条件+否决条件) + 8 常见格局速查表
  - **对星组合**：昌曲/辅弼/魁钺/火铃 5 组，4 条判定规则
  - **空宫借星**：7 场景完整规则（借星不带四化，冲照传导）
  - **流年命宫**：三步法 + 3 条叠加规则
- 多轮对话：4 套关联模板 + 纯文本格式规则（禁 markdown 表格/标题/粗体）
- 风险过滤：5 禁 + 5 缓冲 + 化忌模板 + 桃花专项 + 化权夫妻宫模板
- 安全话术：超出范围标准回复 + 3 个诚实回答场景
- 输出模式：全盘(A) 5 章 + 单主题(B)，结尾强制引导追问
