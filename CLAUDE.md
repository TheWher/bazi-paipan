...
- **快捷输入**：表单顶部支持 `200508190135` 一键填表，回车触发排盘
- **排盘 → 验盘 → 分析** 三步走：用户提交表单 → `/api/paipan` → 渲染命盘（自动加载五行环图/十二长生/大运环图/干支关系图）→ 用户点击"深度分析"→ Agent 输出验盘（~30s）→ 用户确认 → 续接正式批断（4-6min）
- **历史命例**：localStorage 保存最多 20 条，支持自定义命名、删除、一键恢复完整命盘+图表+分析。排盘后自动保存，分析完成自动更新
- **词条弹窗**：42 术语在分析报告中自动高亮（金色虚线下划线），悬停弹出白话解释+经典出处。`injectGlossary()` 用 DOM TreeWalker 注入
- **干支关系图**：SVG 三区布局——上部天干相克（白圈灰线黑字）、中部四柱主体（五行配色）、下部地支合冲刑害（含暗合/自刑/半合/六合/六冲/相害）。按跨度排序，长线在上
- **八字学堂**：页面底部折叠区，42 术语按 9 分类网格排列，自适应布局
- **localStorage 持久化**：`bazi_plate`（排盘结果）、`bazi_analysis`（含续接的完整分析文本）、`bazi_history`（历史列表）
- **分析断线恢复**：`bazi_analysis_pending` 保存待处理状态，切标签页/关页面后自动恢复重试
- **报告打印**：`/report` 页从 localStorage 读取完整累积的 `bazi_analysis` 渲染
- **Markdown 渲染**：自定义 `formatMarkdown()` 正则替换，零依赖
- **交互动画**：背景呼吸光晕、按钮波纹、滚动揭示、分析完成粒子庆祝、卡片悬浮金边脉冲、表格行悬浮浮起、分析章节五行分色
