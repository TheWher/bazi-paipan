...
**Agent 定义文件**：`.claude/agents/traditional-bazi-master.md`（~1,500 行）**被 git 追踪**，随代码一同部署。定义了 9 级递进推理链和 42 个核心概念。采用梁湘润体系（调候第一优先）。

**知识库文件**：`knowledge_base/` 目录下 9 个结构化 JSON（含原有的 `bazi_basics.json` 和 `bazi_extended.json`，以及新增的 7 个领域文件），总大小 ~50KB。`analysis_service.py` 通过 `_load_json_kb()` 按需加载并内存缓存，替代了原先内联 Python dict 的数据管理方式。
