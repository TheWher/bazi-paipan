...
37. **均衡命局"信号等级≠人生重要性"**：王菲复验证明——Agent按信号等级选S/A/B级年份（1981/1975/2017），全不命中（1989出道/1996结婚/2005再婚）。B级冲日支是常见信号、大部分年份都有，无法区分30岁婚变vs 12岁搬家。需命理推理介入（"酉冲卯在30岁≈婚姻事件"）而非纯查表

## Agent 优化策略（2026-06-16 盲测驱动，2026-06-25 迭代）
...
| 姓名 | 四柱 | 极端度 | 排盘验证 |
|------|------|--------|----------|
| 郎朗 | 壬戌 丙午 戊辰 丙辰 | 极端(spread=4) | ✅ |
| 姚明 | 庚申 乙酉 戊子 辛酉 | 极端(spread=5) | ✅ |
| 李娜 | 壬戌 壬寅 庚辰 辛巳 | 均衡(spread=1) | ✅ |

**王菲复验（2026-06-25，stop+后处理）**：Agent选了1993/1998/2017（B级），命中率仍是0/3。回滚"按信号等级排序"规则——均衡命局瓶颈不在查表精度，在"信号等级≠人生重要性"。

### 验盘策略总纲

**验盘不是人格测试，是时间线验证。** 大运+流年的冲合刑害是排盘引擎确定计算的——时间型问题命中率 92%，特征型仅 38%。

| 命局类型 | 极端度 | 验盘策略 |
|----------|--------|----------|
| 极端命局 | 2-3 | 时间锚定为主 + 特征为辅 |
| 均衡命局 | 0-1 | 纯时间线验证（3条全部时间锚定） |
...
- 错误模式：13条（盲测驱动 + 2026-06-24 迭代，模式4.1/4.2已在2026-06-25回滚）
- 流年信号优先级表：S/A/B/C/D/E 6级
- 叙事等级制度：1-5级
- 禁用戏剧化词：7个
- 验盘策略：极端度分流 + 时间线验证
- **均衡命局验盘规则（2026-06-25 修订）**：对照表提供信号参考，但选年必须结合命理推理（信号等级+年龄阶段匹配度）。置信度标签强制标注。信号不足时诚实告知。禁止纯机械按信号等级取top-N
- **大运状态标记规则**：对照表中每步大运标注 🔵当前/✅已走完/⬜未开始，Agent 选年优先选已走完的大运
- **Gate Check + CoVe**：均衡命局 Gate Check 含置信度标签匹配检查 + 选年是否结合推理检查
- **置信度标签**：🔴高=S/A | 🟡中=B/C | 🔵低=D/E/大运交接
...
**新方案**：
1. `stop_sequences=['【验盘完毕】']` — Agent验盘完物理截停，不依赖prompt请求"暂停"
2. `_verify_predictions(analysis_text, plate_dict, current_year)` — Python提取预测年份→查对照表→输出信号等级校验表。支持3种Agent输出格式（`### 第N件`、`**【X置信】**`、自动提取）。含 `<16岁童年预测硬拦截（🚫）`
3. 成本：¥0/次（纯Python），可靠性：排盘引擎硬事实 vs LLM交叉

### 对照表注入管道

`analysis_service.py` 函数：
- `_evaluate_liunian_signal(liunian_ganzhi, ri_ganzhi, yue_zhi, nian_zhi, dayun, year)` → `(level, desc)` — 六级信号判定
- `_build_year_lookup_table(plate_dict, current_year)` → markdown — 逐年全量对照表
- `_verify_predictions(analysis_text, plate_dict, current_year)` → markdown — 后处理硬校验，追加到analysis末尾
- `_call_api(... stop_sequences=list)` — API截停参数

对照表在 `_build_user_message()` 中自动注入排盘数据与分析要求之间。

### 均衡命局规则回滚（2026-06-25）

原因：王菲盲测显示——"强制全扫描+按信号等级选年"把Agent变成了查表机器，均衡命局命中率仍是0/3。Agent机械选S/A/B级年份（1981/1975/2017），全不命中真实事件年份（1989/1996/2005）。

回滚内容：`验盘条数=min(3,可用信号数)`、`按信号等级排序取top-N`、`禁止心算，对照表是唯一权威`、`对照表缺失全部降级`。
