...
# ═══ 十四主星亮度修正（仅修正 iztro 引擎明确错误的位置）═══
# 传统紫微斗数只有庙旺陷三级是公认的。得利平不四级各派不一，
# 强行全覆盖反而引入争议。此表只修正 iztro 明显算错的地方：
# - 星在某宫传统公认庙/旺，iztro 却说陷/平 → 修正
# - 星在某宫传统公认陷，iztro 却说庙/旺 → 修正
# - 中间亮度（得/利/平/不）→ 信任 iztro，不覆盖
_BRIGHTNESS_FIX = {
    '太阴': {'亥': '庙'},  # iztro说陷，传统月朗天门格=庙
    '贪狼': {'巳': '陷', '亥': '陷'},  # iztro可能不准
    '巨门': {'卯': '陷', '酉': '陷'},  # iztro可能不准
    '天机': {'丑': '陷', '未': '陷'},
    '天同': {'丑': '陷', '未': '陷'},
    '廉贞': {'子': '陷', '午': '陷'},
    '七杀': {'丑': '陷', '未': '陷'},
